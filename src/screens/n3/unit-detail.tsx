  ...
  return (
    ...
    >
      <View className="flex-row flex-wrap items-center gap-1 pt-6">
        ...
        <Text className="text-base text-slate-500">N3</Text>
        <Text className="text-base text-slate-500">›</Text>
        <Pressable onPress={() => router.push("/n3" as any)}>
      ...
      <View className="flex-row gap-3">
        <Pressable
          ...
          onPress={() => {
            ...
            const segments = unit.vocabulary.flatMap((item) => {
              const japaneseToPlay = item.hiragana?.trim()
                ? item.hiragana
                : item.japanese;
        ...
        <Pressable
          className="rounded-xl bg-slate-700 px-4 py-3 dark:bg-slate-600"
          onPress={stop}
        >
      ...
      {unit.vocabulary.map((item) => (
        ...
        >
          ...
          <Pressable
            ...
            onPress={() =>
              void playTextSequence([
                {
                  text: item.hiragana?.trim() ? item.hiragana : item.japanese,
                  language: "ja-JP",
                  rate: 0.9,
                },
              ])
          ...
          >
            <Text className="text-center font-semibold text-sakura-700">
              Play {item.japanese}
            </Text>
          </Pressable>
        </View>
