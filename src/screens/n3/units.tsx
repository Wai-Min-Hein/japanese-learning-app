      ...
      [...GENERATED_N3_UNITS]
        ...
        .flatMap((unit) =>
          unit.vocabulary.map((item) => ({
            text: ('hiragana' in item && item.hiragana?.trim() ? item.hiragana : item.japanese) ?? '',
  ...
  return (
    <ScrollView className="flex-1 bg-slate-100 dark:bg-slate-950" contentContainerClassName="gap-4 px-5 pb-5 pt-10">
      <View className="flex-row flex-wrap items-center gap-1 pt-10">
        ...
        <Pressable onPress={() => router.push('/')}>
          <Text className="text-base font-semibold text-sakura-700">N3</Text>
        </Pressable>
        <Text className="text-base text-slate-500">›</Text>
        <Text className="text-base font-semibold text-slate-700 dark:text-slate-300">All Units</Text>
      </View>
      ...
      <View className="gap-3">
        {units.map((unit) => (
          ...
          >
            <Text className="text-base font-semibold text-emerald-700">{unit.title}</Text>
      ...
      <View className="flex-row gap-3">
        ...
        >
          <Text className="text-center text-base font-semibold text-white">
            {isPlaying ? 'Stop Global Vocabulary Audio' : 'Play All Vocabulary (All Units)'}
          </Text>
        </Pressable>
        <Pressable className="rounded-2xl bg-slate-700 px-4 py-4 dark:bg-slate-600" onPress={stop}>
